...
class Penyakit {
	...
	function tampilPenyakit () {
		$data = mysqli_query($this->con, "SELECT * FROM penyakit INNER JOIN pengobatan ON penyakit.ID_HP = pengobatan.ID_HP");
		while ($d = mysqli_fetch_array($data)) {
			$hasil[] = $d;
		}
		return $hasil;
	...
	function tampilketHP($ID_HP) {
		$data = mysqli_query($this->con, "SELECT * FROM pengobatan WHERE ID_HP = '$ID_HP'");
		while ($d = mysqli_fetch_array($data)) {
			$hasil = $d['ketHP'];
		}
		return $hasil;
